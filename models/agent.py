...
class Agent:
    ...
    def train_net(self, batch):
        ...
        for i in range(batch[0].shape[0]):
            target_q[i] = reward_batch[i] if done_batch[i] else reward_batch[i] + params_dqn.gamma * max_next_q[i]

        result = self.q_net.fit(x=action_feature_batch, y=target_q, verbose=0)
        return result.history['loss']

    def save_model(self, episode_count):
        self.q_net.save('./checkpoints/dqn/add_' + str(params_dqn.max_count) + '/model/' + str(episode_count))

    def load_model(self, episode_count):
        self.q_net = tf.keras.models.load_model('./checkpoints/dqn/add_' + str(params_dqn.max_count) + '/model/' + str(episode_count-params_dqn.model_update_cycle))
